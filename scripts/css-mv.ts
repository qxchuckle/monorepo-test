...
import { readdir, cp, stat, readFile, writeFile } from "node:fs/promises";
...
/** UI 包的目录 */
const UI_DIR = join(PKGS_DIR, "ui", "dist");
...
const removeCharsetDeclaration = async (cssContent) => {
...
const moveAndRenameCss = async () => {
  ...
  const writeStream = createWriteStream(join(UI_DIR, "index.css"), "utf-8");
  ...
  const promises = packages.map(async (pkg) => {
    ...
    const dest = join(UI_DIR, `${pkg}.css`);
    await cp(src, dest);
